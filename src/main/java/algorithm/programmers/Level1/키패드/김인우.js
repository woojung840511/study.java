...
function solution(numbers, hand) {
    ...
    // 4가지 방향 , Level1.키패드 이동 한칸은 거리 1
    // numbers = Level1.키패드 (순서대로) , hand = 주 손가락 (두 엄지손가락의 거리가 같다면)
        ...
        for (let finger in fingerposition) {
            if (fingerposition[finger].indexOf(currentpushposition[2]) > -1) { // 키패드를 누른 손가락의 위치
                if (finger.includes("left")) { // 왼쪽손가락범위에 포함된다면
                    ...
                    // 가운데 위치에 있다면 // Level1.키패드 위치에서 계산 // 키패드의 위치를 눕혔다고 생각
...
function solution(numbers, hand) {
    var answer = '';
    let keypadobj = { // 좌표를 통한 Level1.키패드 객체
        1: [1, 1], 2: [1, 2], 3: [1, 3],
        4: [2, 1], 5: [2, 2], 6: [2, 3],
        7: [3, 1], 8: [3, 2], 9: [3, 3],
        '*': [4, 1], 0: [4, 2], '#': [4, 3]
